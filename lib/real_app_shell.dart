...
class _RealAppShellState extends State<RealAppShell> {
  ...
  Widget build(BuildContext context) {
    final pages = [const RealDiscoverScreen(), const ui.KeysScreen(), const ui.MessagesScreen(), const ui.MyProfileScreen()];
    final icons = [
    ...
    return Scaffold(
      ...
      bottomNavigationBar: SafeArea(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(28), boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: .08), blurRadius: 24, offset: const Offset(0, 8))]),
          child: Row(children: List.generate(4, (i) => Expanded(child: InkWell(
            onTap: () => setState(() => index = i),
            child: Padding(padding: const EdgeInsets.symmetric(vertical: 6), child: Column(mainAxisSize: MainAxisSize.min, children: [ui.AppSvg(index == i ? icons[i][1] : icons[i][0], size: 27), const SizedBox(height: 3), Text(labels[i], style: TextStyle(fontSize: 11, fontWeight: index == i ? FontWeight.w800 : FontWeight.w600, color: index == i ? ui.OpenApp.ink : ui.OpenApp.muted))])),
          )))),
        ),
      ),
    );
...
class _RealDiscoverScreenState extends State<RealDiscoverScreen> {
  ...
  void reload() => setState(() { cardIndex = 0; future = OpenBackend.instance.discover(); });
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      Expanded(child: Container(
        ...
        decoration: BoxDecoration(borderRadius: BorderRadius.circular(34), color: ui.OpenApp.soft, image: avatar != null && avatar.isNotEmpty ? DecorationImage(image: NetworkImage(avatar), fit: BoxFit.cover) : null),
        child: Container(
          padding: const EdgeInsets.all(24),
          decoration: BoxDecoration(borderRadius: BorderRadius.circular(34), gradient: LinearGradient(begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Colors.transparent, Colors.black.withValues(alpha: avatar == null || avatar.isEmpty ? .12 : .78)])),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Spacer(),
            Text(name, style: TextStyle(fontSize: 32, fontWeight: FontWeight.w900, color: avatar != null && avatar.isNotEmpty ? Colors.white : ui.OpenApp.ink)),
            if (city.isNotEmpty) Text(city, style: TextStyle(fontSize: 16, color: avatar != null && avatar.isNotEmpty ? Colors.white70 : ui.OpenApp.muted)),
            if (bio.isNotEmpty) Padding(padding: const EdgeInsets.only(top: 8), child: Text(bio, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(color: avatar != null && avatar.isNotEmpty ? Colors.white : ui.OpenApp.ink))),
...
class _CircleAction extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => InkWell(onTap: onTap, borderRadius: BorderRadius.circular(40), child: Container(width: 58, height: 58, decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: .08), blurRadius: 18)]), child: Icon(icon, size: 29)));
}
...
class RealAnswerScreen extends StatefulWidget {
  ...
  @override State<RealAnswerScreen> createState() => _RealAnswerScreenState();
}
...
class _RealAnswerScreenState extends State<RealAnswerScreen> {
  ...
  Future<void> send() async {
    if (answer.text.trim().isEmpty) { setState(() => error = 'Önce cevabını yaz.'); return; }
  ...
  @override Widget build(BuildContext context) => Scaffold(
    ...
    body: SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(24, 8, 24, 26), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
...
class _StateCard extends StatelessWidget {
  ...
  final String title; final String subtitle; final VoidCallback action;
