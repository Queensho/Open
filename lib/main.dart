...
class _SplashScreenState extends State<SplashScreen> {
  ...
  @override
  Widget build(BuildContext context) => const Scaffold(body: Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
...
class _OnboardingScreenState extends State<OnboardingScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(body: SafeArea(child: Column(children: [
        ...
        Expanded(child: PageView.builder(controller: controller, itemCount: pages.length, onPageChanged: (value) => setState(() => page = value), itemBuilder: (_, i) {
          final p = pages[i];
          return Padding(padding: const EdgeInsets.fromLTRB(28, 6, 28, 24), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: List.generate(pages.length, (d) => AnimatedContainer(duration: const Duration(milliseconds: 220), margin: const EdgeInsets.only(right: 7), width: d == page ? 28 : 9, height: 9, decoration: BoxDecoration(color: d == page ? OpenApp.lime : const Color(0xFFE0E0E0), borderRadius: BorderRadius.circular(10))))),
...
class _LockQuestionsScreenState extends State<LockQuestionsScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(backgroundColor: Colors.transparent), body: SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(26, 4, 26, 28), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 18),
        Expanded(child: ListView.separated(itemCount: questions.length, separatorBuilder: (_, __) => const SizedBox(height: 10), itemBuilder: (_, i) {
          final active = selected.contains(i);
          return InkWell(onTap: () => toggle(i), borderRadius: BorderRadius.circular(22), child: Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: active ? OpenApp.lime.withValues(alpha: .16) : OpenApp.soft, borderRadius: BorderRadius.circular(22), border: Border.all(color: active ? OpenApp.lime : Colors.transparent, width: 2)), child: Row(children: [Expanded(child: Text(questions[i], style: const TextStyle(fontWeight: FontWeight.w700))), const SizedBox(width: 12), AppSvg(active ? AppIcons.unlock : AppIcons.lock, size: 28)])));
        })),
        const SizedBox(height: 12),
...
class _AppShellState extends State<AppShell> {
  ...
  @override
  Widget build(BuildContext context) => Scaffold(body: screens[index], bottomNavigationBar: NavigationBar(backgroundColor: Colors.white, indicatorColor: OpenApp.lime.withValues(alpha: .25), selectedIndex: index, onDestinationSelected: (value) => setState(() => index = value), destinations: [
...
class LockedProfile {
  const LockedProfile({required this.name, required this.age, required this.location, required this.distance, required this.bio, required this.questions});
  final String name;
  final int age;
...
class _DiscoverScreenState extends State<DiscoverScreen> {
  final profiles = const [
    LockedProfile(name: 'Deniz', age: 27, location: 'Kadıköy', distance: '2,4 km', bio: 'Kahve, sahil ve uzun yürüyüşler.', questions: ['Bir pazar sabahı seni nerede bulurum?', 'Seni güldüren küçük şey ne?', 'Plansız bir günün nasıl geçer?']),
    LockedProfile(name: 'Ece', age: 25, location: 'Beşiktaş', distance: '4,1 km', bio: 'Konser, analog fotoğraf ve yeni şehirler.', questions: ['Son anda aldığın en güzel karar neydi?', 'Bir şarkı seni hangi ana götürür?', 'İlk buluşmada en çok neye dikkat edersin?']),
    LockedProfile(name: 'Selin', age: 29, location: 'Şişli', distance: '6,8 km', bio: 'İyi yemek, kötü espriler ve spontane planlar.', questions: ['Hafta sonu senin için mükemmel nasıl geçer?', 'Birine hemen güvenmeni sağlayan şey?', 'Seni gerçekten heyecanlandıran şey ne?']),
  ];
  int profileIndex = 0;
  int questionIndex = 0;
  ...
  @override
  Widget build(BuildContext context) => SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(20, 20, 20, 14), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
          const Expanded(child: PageTitle('Keşfet', 'Fotoğraf kilitli. Önce bir soruyu seç ve cevapla.')),
          Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(color: OpenApp.soft, borderRadius: BorderRadius.circular(18)), child: Text('${profileIndex + 1}/${profiles.length}', style: const TextStyle(fontWeight: FontWeight.w900))),
        ]),
        const SizedBox(height: 18),
        Expanded(child: AnimatedSwitcher(duration: const Duration(milliseconds: 260), child: Container(key: ValueKey(profileIndex), width: double.infinity, padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: OpenApp.soft, borderRadius: BorderRadius.circular(32), border: Border.all(color: const Color(0xFFE9E9E5))), child: Column(children: [
          Row(children: [
            IconButton(onPressed: () => move(-1), icon: const Icon(Icons.chevron_left_rounded, size: 30)),
            Expanded(child: Center(child: AppSvg(AppIcons.lock, size: 118, card: true))),
            IconButton(onPressed: () => move(1), icon: const Icon(Icons.chevron_right_rounded, size: 30)),
          ]),
          const SizedBox(height: 16),
          Text('${profile.name}, ${profile.age}', style: const TextStyle(fontSize: 30, fontWeight: FontWeight.w900)),
          const SizedBox(height: 6),
          Text('${profile.location} • ${profile.distance}', style: const TextStyle(color: OpenApp.muted, fontWeight: FontWeight.w700)),
          const SizedBox(height: 10),
          Text(profile.bio, textAlign: TextAlign.center, style: const TextStyle(fontSize: 15.5)),
          const SizedBox(height: 20),
          const Align(alignment: Alignment.centerLeft, child: Text('Bir kilit sorusu seç', style: TextStyle(fontSize: 17, fontWeight: FontWeight.w900))),
          const SizedBox(height: 10),
          Expanded(child: ListView.separated(physics: const BouncingScrollPhysics(), itemCount: profile.questions.length, separatorBuilder: (_, __) => const SizedBox(height: 9), itemBuilder: (_, i) {
            final active = i == questionIndex;
            return InkWell(onTap: () => setState(() => questionIndex = i), borderRadius: BorderRadius.circular(18), child: AnimatedContainer(duration: const Duration(milliseconds: 180), padding: const EdgeInsets.all(15), decoration: BoxDecoration(color: active ? OpenApp.lime.withValues(alpha: .18) : Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: active ? OpenApp.lime : const Color(0xFFE5E5E2), width: active ? 2 : 1)), child: Row(children: [AppSvg(active ? AppIcons.key : AppIcons.question, size: 26), const SizedBox(width: 12), Expanded(child: Text(profile.questions[i], style: TextStyle(fontWeight: active ? FontWeight.w800 : FontWeight.w600)))])));
          })),
          const SizedBox(height: 14),
          FilledButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => AnswerScreen(profile: profile, question: profile.questions[questionIndex]))), child: const Text('Bu soruyu cevapla')),
        ]))),
        const SizedBox(height: 12),
        Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(profiles.length, (i) => AnimatedContainer(duration: const Duration(milliseconds: 180), margin: const EdgeInsets.symmetric(horizontal: 4), width: i == profileIndex ? 24 : 8, height: 8, decoration: BoxDecoration(color: i == profileIndex ? OpenApp.lime : const Color(0xFFD8D8D4), borderRadius: BorderRadius.circular(10))))),
      ])));
}
...
class _AnswerScreenState extends State<AnswerScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(backgroundColor: Colors.transparent), body: SafeArea(child: Padding(padding: const EdgeInsets.fromLTRB(24, 8, 24, 26), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 10),
        const Text('Seçtiğin soruya vereceğin cevap karşı tarafa anahtar olarak gidecek.', style: TextStyle(color: OpenApp.muted, height: 1.4)),
        const SizedBox(height: 22),
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Widget build(BuildContext context) => Scaffold(appBar: AppBar(backgroundColor: Colors.transparent, title: Text(widget.name)), body: SafeArea(child: Padding(padding: const EdgeInsets.all(20), child: Column(children: [
        Expanded(child: ListView(children: [
          Align(alignment: Alignment.centerLeft, child: Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: OpenApp.soft, borderRadius: BorderRadius.circular(18)), child: const Text('Selam! Cevabını sevdim 😊'))),
          ...messages.map((m) => Align(alignment: Alignment.centerRight, child: Container(margin: const EdgeInsets.only(bottom: 10), padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: OpenApp.lime, borderRadius: BorderRadius.circular(18)), child: Text(m)))),
        ])),
