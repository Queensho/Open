...
Widget _appShell() => Builder(
      builder: (context) => RealAppShell(
        onSignedOut: () => Navigator.of(context, rootNavigator: true).pushAndRemoveUntil(
...
class _SplashState extends State<_Splash> {
  ...
  Future<void> _route() async {
    ...
    final user = OpenBackend.instance.client.auth.currentUser;
...
class _OnboardingState extends State<_Onboarding> {
  ...
  void next() {
    if (page < pages.length - 1) {
      ...
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const _RegisterScreen()));
    }
  }
            ...
            children: [
              Align(
                ...
                child: Padding(
                  ...
                  child: TextButton(
                    onPressed: () => Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const _RegisterScreen())),
                    child: const Text('Atla', style: TextStyle(color: ui.OpenApp.ink, fontWeight: FontWeight.w800)),
                  ),
                ),
              ),
              Expanded(
                  ...
                  itemBuilder: (_, i) {
                    ...
                    return Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Expanded(child: Center(child: ui.AppSvg(item.$3, size: 150, card: true))),
                          Text(item.$1, style: const TextStyle(fontSize: 34, height: 1.05, fontWeight: FontWeight.w900)),
                          const SizedBox(height: 12),
                          Text(item.$2, style: const TextStyle(color: ui.OpenApp.muted, fontSize: 16, height: 1.4)),
                          const SizedBox(height: 20),
                          FilledButton(onPressed: next, child: Text(i == pages.length - 1 ? 'Hadi başlayalım' : 'Devam et')),
                        ],
                      ),
                    );
                  },
...
class _RegisterScreenState extends State<_RegisterScreen> {
  ...
  Future<void> register() async {
    ...
    setState(() { busy = true; error = null; });
    try {
      final result = await OpenBackend.instance.registerEmail(email.text, password.text);
      if (result.session == null) {
        if (mounted) setState(() => error = 'E-posta doğrulaması açık. Supabase ayarından kapatılmalı.');
        return;
      }
      if (!mounted) return;
      Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => const _ProfileScreen()), (_) => false);
    } catch (e) {
      if (mounted) setState(() => error = 'Kayıt oluşturulamadı: $e');
    } finally {
      if (mounted) setState(() => busy = false);
    }
  }
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: ['Kadın', 'Erkek', 'Belirtmek istemiyorum']
                      .map((item) => ChoiceChip(label: Text(item), selected: gender == item, selectedColor: ui.OpenApp.lime, onSelected: (_) => setState(() => gender = item)))
                      .toList(),
                ),
...
class _QuestionsScreenState extends State<_QuestionsScreen> {
  ...
  Future<void> finish() async {
    ...
    try {
      ...
      if (!mounted) return;
      Navigator.pushAndRemoveUntil(context, MaterialPageRoute(builder: (_) => _appShell()), (_) => false);
    } catch (e) {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                      ...
                      return InkWell(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            color: active ? ui.OpenApp.lime.withValues(alpha: .16) : ui.OpenApp.soft,
                            borderRadius: BorderRadius.circular(22),
                            border: Border.all(color: active ? ui.OpenApp.lime : Colors.transparent, width: 2),
                          ),
                          child: Row(
                            children: [
                              Expanded(child: Text(questions[i], style: const TextStyle(fontWeight: FontWeight.w700))),
                              ui.AppSvg(active ? ui.AppIcons.unlock : ui.AppIcons.lock, size: 28),
                            ],
                          ),
                        ),
                      );
                ...
                FilledButton(
                  onPressed: selected.length == 3 && !busy ? finish : null,
                  child: Text(busy ? 'Kaydediliyor...' : (selected.length == 3 ? 'Profili tamamla' : '${selected.length}/3 soru seçildi')),
                ),
              ],
            ),
          ),
